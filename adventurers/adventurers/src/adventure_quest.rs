...
pub enum GameEvent {
    BlockEvent { target: Blocks, count: u8 },
    CollectEvent { target: char },
}
...
impl std::fmt::Display for ParallelQuest {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        res += &format!(
            "You should complete at least {} of these quests: \n",
            self.count
        );
        for quest in &self.quests {
            res += &format!("{}", quest);
...
impl GiveQuest {
    fn give_value(&self) -> Box<dyn Quest<GameEvent>> {
        match self {
            GiveQuest::Q1 => Box::from(SingleQuest {
                target: Blocks::Sand,
                count: 1,
                repeat: 5,
                completed: false,
            ...
            GiveQuest::Q2 => Box::from(SequenceQuest {
                quests: vec![
                    Box::from(SingleQuest::<char> {
                        target: 'x',
                        count: 1,
                        repeat: 5,
                        completed: false,
            ...
            GiveQuest::Q3 => Box::from(ParallelQuest {
                quests: vec![
                    Box::from(SequenceQuest {
                        quests: vec![
                            Box::from(SingleQuest {
                                target: Blocks::Sand,
                                count: 1,
                                repeat: 5,
                                completed: false,
...
pub fn initialize_quest(
    quest_num: &String,
) -> Result<Box<dyn Quest<GameEvent>>, GameInitializationError> {
    if *quest_num == "q1" {
