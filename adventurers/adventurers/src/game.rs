...
impl MyGame {
    ...
    pub fn player_in_viewport(&self, game: &Game) -> bool {
        ...
        if player_rel_x_pos < 0
            || !(0..=77).contains(&player_rel_x_pos)
            || !(0..=21).contains(&player_rel_y_pos)
        {
            return false;
        }
        true
    ...
    pub fn next_block_is_barrier(&self, movement: &Coordinate) -> bool {
        ...
        matches!(
            self.map.get(&(
                curr_player_block.x + movement.x,
                curr_player_block.y + movement.y,
            )),
            Some(Blocks::Barrier)
        )
    }
            ...
            Some(style) => {
                ...
                if self.curr_block_is_sign() {
                    game.set_screen_char(
                        self.player.get_x_pos(),
                        self.player.get_y_pos(),
        ...
        match next_style {
            Some(style) => {
                game.set_screen_char(
                    self.player.get_x_pos(),
                    self.player.get_y_pos(),
                    Some(style.character(self.player.repr)),
                );
            }
            None => {
                game.set_screen_char(
                    self.player.get_x_pos(),
                    self.player.get_y_pos(),
                    Some(StyledCharacter::from(self.player.repr)),
                );
            }
        }
    }
            ...
            match block {
                ...
                Blocks::Cinderblock => {
                    game.set_screen_char(
                        ...
                        *y,
                        Some(
                            StyledCharacter::from(' ').style(
                                GameStyle::new().background_color(Some(GameColor::LightRed)),
                            ),
                        ),
                    );
                }
                ...
                Blocks::Object(character) => {
                    game.set_screen_char(
                        *x,
                        *y,
                        Some(
                            StyledCharacter::from(*character)
    ...
    pub fn generate_block_event(&mut self, curr_block: Blocks) -> MyGameEvent {
        ...
        MyGameEvent::BlockEvent {
            target: curr_block,
            count: self.player.continue_steps,
        }
    }
}
...
impl Controller for MyGame {
    ...
    fn on_event(&mut self, game: &mut Game, event: GameEvent) {
        if let SimpleEvent::Just(KeyCode::Char(ch)) = event.into() {
            ...
            match game.get_message() {
                Some(msg) => {
                    if msg.text == *"You are drowned." || msg.text == *"YOU WIN!!!" {
                        game.end_game();
                    } else {
            ...
            match self.get_curr_block() {
                Some(block) => self.player.prev_block = Some(block.clone()),
            ...
            let event = match curr_block {
                Blocks::Object(target) => Box::from(self.generate_char_event(target)),
            ...
            if let Blocks::Object(_) = curr_block {
                self.map
                    .remove(&(self.player.get_x_pos(), self.player.get_y_pos()));
            }
        }
    }
